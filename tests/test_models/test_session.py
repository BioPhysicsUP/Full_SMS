...
class TestFileMetadata:
    ...
    def test_create_metadata(self) -> None:
        ...
        assert metadata.num_particles == 10
...
class TestChannelSelection:
    ...
    def test_create_selection(self) -> None:
        ...
        selection = ChannelSelection(particle_id=1, channel=1)

        assert selection.particle_id == 1
        assert selection.channel == 1

    def test_default_channel(self) -> None:
        """Default channel is 1."""
        selection = ChannelSelection(particle_id=5)

        assert selection.channel == 1

    def test_channel_2(self) -> None:
        """Can select channel 2."""
        selection = ChannelSelection(particle_id=1, channel=2)
    ...
    def test_invalid_channel_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="channel must be 1 or 2"):
            ChannelSelection(particle_id=1, channel=3)

        with pytest.raises(ValueError, match="channel must be 1 or 2"):
            ChannelSelection(particle_id=1, channel=0)
...
class TestProcessingState:
    ...
    def test_start_with_message(self) -> None:
        ...
        processing.start("CPA", message="Analyzing particle 1...")

        assert processing.message == "Analyzing particle 1..."
...
class TestSessionState:
    ...
    @pytest.fixture
    def sample_particle(self, sample_channel: ChannelData) -> ParticleData:
    ...
    def test_num_particles(self, sample_particle: ParticleData) -> None:
    ...
    def test_get_particle(self, sample_particle: ParticleData, sample_channel: ChannelData) -> None:
        """get_particle returns particle by ID."""
        ...
        state = SessionState()
        state.particles = [sample_particle, particle2]

        assert state.get_particle(1) is sample_particle
        assert state.get_particle(2) is particle2
        assert state.get_particle(999) is None

    def test_get_current_particle(self, sample_particle: ParticleData) -> None:
        """get_current_particle returns selected particle."""
        state = SessionState()
        state.particles = [sample_particle]

        # No selection
        assert state.get_current_particle() is None

        # With selection
        state.select(1)
        assert state.get_current_particle() is sample_particle

    def test_select(self, sample_particle: ParticleData) -> None:
        ...
        assert state.current_selection.particle_id == 1
        assert state.current_selection.channel == 1
        assert state.has_selection is True
        assert len(state.selected) == 1

    def test_select_multiple(self, sample_particle: ParticleData, sample_channel: ChannelData) -> None:
        """Selecting multiple particles adds to list."""
        particle2 = ParticleData(
            id=2,
            name="Particle 2",
            tcspc_card="SPC-150",
        ...
        assert state.current_selection.particle_id == 2

    def test_select_same_particle_no_duplicate(self, sample_particle: ParticleData) -> None:
        """Selecting same particle again doesn't duplicate in list."""
        state = SessionState()
        state.particles = [sample_particle]
    ...
    def test_select_channel_2(self, sample_particle: ParticleData, sample_channel: ChannelData) -> None:
        """Can select channel 2."""
        dual_particle = ParticleData(
            id=1,
            name="Particle 1",
            tcspc_card="SPC-150",
            channelwidth=0.012,
        ...
        state = SessionState()
        state.particles = [dual_particle]

        state.select(1, channel=2)

        assert state.current_selection.channel == 2

    def test_clear_selection(self, sample_particle: ParticleData) -> None:
    ...
    def test_set_and_get_levels(self, sample_particle: ParticleData, sample_level: LevelData) -> None:
    ...
    def test_get_current_levels(self, sample_particle: ParticleData, sample_level: LevelData) -> None:
