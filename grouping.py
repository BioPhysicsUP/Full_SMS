"""Module for handling performing Agglomerative Hierarchical Clustering
Algorithm.
...
class Calcs:
    ...
    def setup(self, particle):
        ...
        """
        assert hasattr(particle,
                       'levels'), 'No levels have been resolved to merge.'
...
def g(n: np.int, cap_i: np.float, cap_t: np.float) -> np.float:
    """
    Poisson probability of detection n number of photons in a level given I
    and T. Eq (2)
    ...
    return np.exp(
        n * np.log(cap_i * cap_t) - (cap_i * cap_t) - np.float(lgamma(n + 1)))


class AHCA:
    """
    Class for executing Agglomerative Hierarchical Clustering Algorithm and
    storing the results.
    """
    ...
    def run_grouping(self):
        ...
        for i in range(self._calcs.cap_j):
            self.ahg()
            self.em()
            # print(f"Number of States: {self._calcs.cap_g},
            # BIC: {self._calcs.bic[-1]}")
            all_em_p_mj.append(self._calcs.em_p_mj)
            if plotting_on:
                row = i // columns
                column = i - columns * (i // columns)
                title = f"i={i + 1}, #S={self._calcs.cap_g}"
                ax_p_mj[row, column].pcolor(self._calcs.p_mj, edgecolors='k',
                                            linewidths=0.1, cmap='GnBu')
                ax_p_mj[row, column].set_title(title)
                ax_em_p_mj[row, column].pcolor(self._calcs.em_p_mj,
            ...
            if j not in self._calcs.merged:
                ...
                for m in range(j + 1, cap_j + 1):  # row
                    if m not in self._calcs.merged:
                        ...
                        merge_mj[m, j] = (n_m + n_j) * np.log(
                            (n_m + n_j) / (cap_t_m + cap_t_j))\
                                         - n_m * np.log(
                            n_m / cap_t_m) - n_j * np.log(n_j / cap_t_j)
    ...
    def em(self):
        ...
        while max_diff > 1E-10 and i < 500:
            ...
            for j in range(cap_j + 1):
                ...
                #                       for m in range(cap_j + 1) if m not
                #                       in self._calcs.merged])
                # if j not in self._calcs.merged:
                n_j = n[j]
                cap_t_j = cap_t[j]
                denom_j[j] = np.sum(
                    [p_m[m] * g(n_j, cap_i_m[m], cap_t_j) for m in
                     range(cap_j + 1) if m not in self._calcs.merged])
                # denom_j[j] = np.sum([p_m[m]*g(n_j, cap_i_m[m], cap_t_j)
                # for m in range(cap_j + 1)
                #                   if m not in self._calcs.merged])
            ...
            for m in range(cap_j + 1):
                if m not in self._calcs.merged:
                    ...
                    if g_value != 0:
                        ...
                        try:
                            log_l_em_mj[m, j] = p_mj[m, j] * np.log(
                                p_m[m] * g_value)
        ...
        bic = 2 * log_l_em - (2 * n_g - 1) * np.log(
            cap_n_cp) - cap_n_cp * np.log(cap_n)
