...
class MatplotlibWidget(Canvas):
    ...
    def __init__(self, parent=None, title='', xlabel='', ylabel='',
                 ...
                 width=4, height=3, dpi=None, hold=False):
        if dpi is None:
            dpi = rcParams.__getitem__('figure.dpi')
        self.figure = Figure(figsize=(width, height), dpi=dpi)
        window_color = parent.palette().color(QPalette.Window)
        rgba_color = (window_color.red() / 255, window_color.green() / 255, window_color.blue() / 255, 1)
        self.figure.patch.set_facecolor(rgba_color)
